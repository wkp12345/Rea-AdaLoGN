...
os.environ["WANDB_API_KEY"] = 'bd6d1a692dfeba0b056fea28e8766ea682ea9395'  # 将引号内的+替换成自己在wandb上的一串值
os.environ["WANDB_MODE"] = "offline"  # 离线  （此行代码不用修改）
# 最后训练完后会出现wandb sync wandb / xxx，按照提示同步即可
...
def main():
    ...
    def compute_metrics(p: EvalPrediction) -> Dict:
        ...
        if Config.eval_test:
            ...
            dev_acc = accuracy_score(p.label_ids[:dev_len], preds[:dev_len])    # 验证集的准确率
            ...
            if not os.environ['DATASET_DIR'] == 'ReclorDataset':
                ...
                _p = [[math.e ** xx for xx in x] for x in _preds[:500]]    # dev集的结果
                dev_loss = sum([-math.log(x[p.label_ids[index]] / sum(x)) for index, x in enumerate(_p)]) / len(_p)
            ...
            while os.path.exists(os.path.join('results', training_args.run_name, save_path)):
                count += 1
                save_path = f'test_{dev_acc}_{count}.npy'
            np.save(os.path.join('results', training_args.run_name, save_path), preds[dev_len:])  # test集的结果
    ...
    os.environ["WANDB_PROJECT"] = "adalogn"
    ...
    if training_args.do_eval:
        ...
        logger.info("*** Predict ***")
        results = trainer.predict(pre_dataset)
        output_eval_file = os.path.join(training_args.output_dir, "pre_results.txt")
        with open(output_eval_file, "w") as writer:
            logger.info("***** Pre results *****")
            logger.info("  %s = %s", "predictions", results.predictions)
            logger.info("  %s = %s", "label_ids", results.label_ids)
