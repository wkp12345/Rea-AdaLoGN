...
class LabelSmoothingCrossEntropyLoss(nn.Module):
    ...
    def forward(self, x, target):
        ...
        loss = self.confidence * nll_loss + self.smoothing * smooth_loss   # （17）
        # l2_loss = 0.0
        # for param in self.parameters():
        #     l2_loss += torch.sum(param.pow(2))
        # loss += self.weight_decay * l2_loss  # add L2 regularization  + 二难 ada17; + SELU ada18
        return loss.mean()
...
class RobertaAdaLoGN(RobertaPreTrainedModel):
    ...
    def _get_split_representation(self, input_ids, last_hidden_states, graph_batch_num, node_intervals,
        ...
        node_representations = [torch.stack(           # （9）
    ...
    def _get_split_origin_context_answer_representation(self, input_ids, last_hidden_states):
        ...
        context_origin_representations = [        # （2）
    ...
    def forward(
        ...
        bert_outputs = self.roberta(    # （8）
            input_ids,
            attention_mask=attention_mask,
            token_type_ids=token_type_ids,
            return_dict=True,
        )
        bert_outputs_origin = self.roberta(    # （1）
            input_ids_origin,
        ...
        # # 获取处理后的文章和选项的表示
        # context_representations, question_representations, answer_representations = self._get_split_origin_context_answer_representation(
        #      input_ids, last_hidden_states)
        #
        # # 把文章和选项的初始表示和处理后的表示进行融合在进行预测   AdaLoGN_Reclor5
        # context_origin_representations = (context_origin_representations + context_representations)/2
        # answer_origin_representations = (answer_origin_representations + answer_representations)/2

        inputs = torch.cat([context_origin_representations, question_origin_representations, answer_origin_representations,
                       graphs_representations],
                      dim=-1)
        outputs = self.output_layer1(inputs)
        outputs = torch.tanh(outputs)
        outputs = self.output_layer2(outputs).view(-1, 4)   # (16)

        # # 加入处理后的的文章和选项的表示进行一个答案预测然后和用初始表示的答案预测结果进行融合
        # inputs1 = torch.cat([context_representations, question_origin_representations, answer_representations,
        #                graphs_representations],
        #               dim=-1)
        # outputs1 = self.output_layer1(inputs1)
        # outputs1 = torch.tanh(outputs1)
        # outputs1 = self.output_layer2(outputs1).view(-1, 4)
        #
        # # # 对outputs和outputs1进行求平均    AdaLoGN_Reclor2
        # # outputs = torch.mean(torch.stack([outputs, outputs1]), dim=0)
        #
        # # 对outputs和outputs1两个结果取最大值   AdaLoGN_Reclor6
        # outputs = torch.maximum(outputs, outputs1)
