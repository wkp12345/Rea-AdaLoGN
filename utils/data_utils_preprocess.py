...
def load_data():
    ...
    pre_dataset = json.load(open(f'{dataset_dir}/predict.json', 'r', encoding='utf-8'))
    pre_dataset = dict([(data['id_string'], data) for data in pre_dataset])

    return train_dataset, val_dataset, test_dataset, pre_dataset


train_dataset, val_dataset, test_dataset, pre_dataset = load_data()

dataset = {**train_dataset, **val_dataset, **test_dataset, **pre_dataset}
...
def load_all_subsentences_with_logic():
    ...
    for filename in os.listdir(f'{dataset_dir}/flat'):
        ...
        flat_datas = list(filter(lambda x: x.strip() != '', flat_datas))   # 去除首尾空格或换行符
        new_flat_datas = []
...
def get_element_map(text):
    ...
    element_map = {}    # 创建一个空字典，用于存储元素映射信息
    for sentence in EDUs[text]['sentences']:
        element_map = {**element_map, **sentence['elementMap']}   # 将句子的元素映射信息添加到element_map字典中
    del_hash_ids = []   # 创建一个空列表，用于存储需要删除的元素映射ID
    for hash_id in element_map:    # 遍历element_map字典中的所有元素映射ID
        if len(element_map[hash_id]['text'].split(' ')) < 3 and len(element_map) > 1:    # 如果元素映射的文本中的词数小于3，并且element_map中元素映射ID数量大于1，则将该ID添加到del_hash_ids列表中
            del_hash_ids.append(hash_id)
    for hash_id in del_hash_ids:
        del element_map[hash_id]
    # 将element_map字典转换为字符串形式，并将需要删除的元素映射ID替换为'deleted'字符串
    element_map_str = json.dumps(element_map)
    for hash_id in del_hash_ids:
        element_map_str = element_map_str.replace(hash_id, 'deleted')
    return edu_merge(json.loads(element_map_str))   # 将处理后的元素映射信息合并并返回
...
def construct_relation_graph_new(id: str):
    ...
    for __index, sentence in enumerate(answers):   # 遍历A,B,C,D四个选项，构建关系图
        ...
        answer_edus = get_element_map(answer_sentence)   # 将answer映射到edus上
        edus = {**context_edus, **answer_edus}   # 将context和answer映射到edus上

        for hash_id in edus:
            main_chains_sentences.append(hash_id)
        sentences_a = list(context_edus.keys())  # 添加context_edus中所有key到sentences_a
        sentences_b = list(answer_edus.keys())   # 添加answer_edus中所有key到sentences_b
        ...
        # 将句子格式化为特定格式，并分别将格式化后的句子添加到对应列表中
        ...
        edges_a, edges_b = edges[0], edges[1]  # 将边的起始和终止节点分别存储到两个列表中
        for index1 in range(nodes_num):
            ...
            for index2 in range(index1 + 1, nodes_num, 1):
                ...
                if pylev.levenschtein(sentence_index1.split(' '), sentence_index2.split(' ')) > 2:
                    continue
                if sentence_index1 in get_from_new_not_sentence_map(  # 如果两个句子之间有NOT关系，则跳过
                        sentence_index2) or sentence_index2 in get_from_new_not_sentence_map(sentence_index1):
                    if [index1, index2] in [[a, b] for a, b in zip(edges_a, edges_b)]:    # 如果边已存在，则跳过
                        continue
...
def get_edge_norm(edge_relations, graph, graph_edges):
    ...
    for i in range(graph.num_edges()):
        in_node_id = graph_edges[1][i]  # 终点id
        current_edge_type = edge_relations[i]  # 当前边的类型
        current_edge_type_count = 0   # 当前类型边的数量
        for j in range(graph.num_edges()):
            if graph_edges[1][j] != in_node_id:  # 找终点id相同的边
                continue
            if edge_relations[j] == current_edge_type:  # 找类型相同的边
                current_edge_type_count += 1
        edge_norm.append(1.0 / current_edge_type_count)  # 规范化
    return edge_norm
...
def merge(graph: dgl.graph, relations, merge_relation_type, node_sentence_a_len, forced_merge=False,
    ...
    for node_id in range(graph.num_nodes()):
        if node_id in merge_nodes or node_id - 1 in merge_nodes:  # 如果节点ID已经在merge_nodes中，或者该节点是前一个节点的ID，则跳过该节点
            continue
        ...
        if not forced_merge:
            ...
            loop_count = 0
            node_id = -1
            # 随机选择一个节点ID，直到找到合适的节点或者循环次数超过图中节点数的一半
        ...
        is_connected = False
        # 判断该节点和下一个节点是否已经相连，以及是否存在其他的边类型
...
def construct_relation_graph_merger_nodes(id, show_graph=False, graph_type=4, context=None, answers=None,
    ...
    answers：答案。
    graphs：多个关系图。
    _node_sentences_a：关系图A的节点文本列表。
    _node_sentences_b：关系图B的节点文本列表。
    relations：多个关系图的关系列表。
    selected_i：选定的图的索引。
    merge_4：是否合并四个节点。
    return_merge_nodes：是否返回合并的节点。
    forced_merge：是否强制合并。
    base_node_ids：基本节点的编号列表。

    返回值：
    如果return_merge_nodes为True，返回上下文文本、答案、多个关系图、关系图中每个节点的文本、多个关系图的关系列表、关系的权重以及合并的节点；
    否则，返回上下文文本、答案、多个关系图、关系图中每个节点的文本、多个关系图的关系列表和关系的权重。
    """
    if context is None:
...
def construct_relation_graph_extension(id: str, context=None, graphs=None, node_sentences_a=None,
    ...
    for i in range(4):   # 每个选项一个图，循环4次
        ...
        for relation_index in range(len(relations[i])):     # 构造性二难规则+gnn3 ada10;构造性二难规则+droupout=0.2 ada14
            if len(trans_exten_edge_ids[i]) > Config.extension_padding_len:   # 构造性二难规则+extension_padding_len=4  ada9
                break
            if relations[i][relation_index] == or_relation_id:  # 构造性二难规则((ui → uj)∧(um → un))∧(ui ∨ um)->(uj ∨ un) ada8  extension_padding_len=3
                ...
                for edge_index in range(len(graphs[i].edges()[0].numpy().tolist())):
                    if graphs[i].edges()[0].numpy().tolist()[edge_index] == edge_a and relations[i][edge_index] == imp_relation_id:
                        for edge_index1 in range(len(graphs[i].edges()[0].numpy().tolist())):
                            if graphs[i].edges()[0].numpy().tolist()[edge_index1] == edge_b and relations[i][edge_index1] == imp_relation_id:
                                trans_exten_edge_ids[i].append(
                                    [graphs[i].edges()[1].numpy().tolist()[edge_index], graphs[i].edges()[1].numpy().tolist()[edge_index1],
                                     relations[i][relation_index]])

        for relation_index in range(len(relations[i])):    # ada19 不用邻接传输
            if len(trans_exten_edge_ids[i]) > Config.extension_padding_len:
                break
            if relations[i][relation_index] == other_relation_id:    # 邻接传输规则((ui * uj ) ∧ (ui ∼ uk)) -> (uk * uj )
                edge_a, edge_b = graphs[i].edges()[0].numpy().tolist()[relation_index], \
                                 graphs[i].edges()[1].numpy().tolist()[relation_index]
                for edge_index in range(len(graphs[i].edges()[0].numpy().tolist())):
        ...
        for _ in range(max_extension_depth - 1):
            ...
            for relation_index in range(len(relations[i])):
                if relations[i][relation_index] == imp_relation_id:   # 假言三段论((ui → uj ) ∧ (uj → uk)) -> (ui → uk)
                    edge_a, edge_b = graphs[i].edges()[0].numpy().tolist()[relation_index], \
                                     graphs[i].edges()[1].numpy().tolist()[relation_index]
                    for edge_index in range(len(graphs[i].edges()[0].numpy().tolist())):
        ...
        # for relation_index in range(len(relations[i])):       # 构造性二难规则 extension_padding_len=3 0425_ada8
        #     if len(trans_exten_edge_ids[i]) > Config.extension_padding_len:
        #         break
        #     if relations[i][relation_index] == or_relation_id:
        #         edge_a, edge_b = graphs[i].edges()[0].numpy().tolist()[relation_index], \
        #                          graphs[i].edges()[1].numpy().tolist()[relation_index]
        #         for edge_index in range(len(graphs[i].edges()[0].numpy().tolist())):
        #             if graphs[i].edges()[0].numpy().tolist()[edge_index] == edge_a and relations[i][edge_index] == imp_relation_id:
        ...
        for relation_index in range(len(relations[i])):
            if len(cont_exten_node_ids[i]) + len(trans_exten_edge_ids[i]) > Config.extension_padding_len:
                break
            if relations[i][relation_index] == imp_relation_id:    # 否定规则 (ui → uj ) -> (¬uj → ¬ui)
                edge_a, edge_b = graphs[i].edges()[0].numpy().tolist()[relation_index], \
                                 graphs[i].edges()[1].numpy().tolist()[relation_index]
        ...
        edges_a, edges_b = graphs[i].edges()[0].numpy().tolist(), graphs[i].edges()[1].numpy().tolist()
        for edge_id in tmp_edges_id:      # 将已经处理过的边的关系值设置为 -1
