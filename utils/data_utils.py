...
class DataProcessor:
    ...
    def get_predict_examples(self, data_dir):
        """Gets a collection of `InputExample`s for the predict set."""
        raise NotImplementedError()
...
class ProcessorBertLogiGraph(DataProcessor):
    ...
    def get_predict_examples(self, data_dir):
        """See base class."""
        logger.info("LOOKING AT {} test".format(data_dir))
        return self._create_examples(f'{data_dir}/predict.json', "predict")
            ...
            if cont_exten_node_ids is not None:
                ...
                for index in range(len(exten_node_ids)):
                    for inner_index in range(len(exten_node_ids[index])):
                        ...
                        # 将 exten_node_ids[index][inner_index] 扩充到长度为 4，不足的地方用 Config.extension_padding_value 填充
                    ...
                    # 将 exten_node_ids[index] 扩充到长度为 Config.extension_padding_len，不足的地方用 [Config.extension_padding_value] * 4 填充
                ...
                for index in range(len(exten_edge_ids)):
                    ...
                    # 将 exten_edge_ids[index] 扩充到长度为 Config.extension_padding_len，不足的地方用 [Config.extension_padding_value] * 3 填充
...
class DatasetBertLogiGraph(Dataset):
    ...
    def __init__(
        ...
        if os.path.exists(cached_features_file) and not overwrite_cache and mode != Split.predict:
            ...
            elif mode == Split.predict:
                examples = processor.get_predict_examples(data_dir)
            else:
                raise NotImplementedError
            logger.info("Training examples: %s", len(examples))
            self.features = convert_examples_to_features_graph_with_origin_rgcn(   # 用原始rgcn把例子转换为特征图
                examples,
                label_list,
                max_seq_length,
...
def tokenizer_encode(example, label_map, max_length, tokenizer):
    ...
    for index, context in enumerate(example.context):
        ...
        ending_origin = example.endings_origin[index]    # answers
        ...
        inputs_origin = tokenizer(           # 转换成id序列
            context_origin,
